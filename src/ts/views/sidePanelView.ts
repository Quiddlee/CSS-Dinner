...
import { SidePanelData } from '../types/interfaces';

const parentElement = document.createElement('ul');
const navWrapper = document.createElement('nav');
let data: SidePanelData;

const generateMarkup = () => {
  const levels = [];

  for (let i = 0; i < data.numOfLevels; i += 1) {
    const isCurrLvl = data.completedLevels.find(
      ({ level: levelNum }) => levelNum === i,
    );

    const level = `
      <li class="${CssClasses.SIDE_PANEL_LEVEL}" data-lvl="${i}">
        <span class="${CssClasses.SIDE_PANEL_LEVEL_FILL}  ${
      i === data.currLevel ? CssClasses.SIDE_PANEL_LEVEL_FILL_ACTIVE : ''
    }"></span>
        <button class="${CssClasses.SIDE_PANEL_LEVEL_BTN} ${CssClasses.BTN}">
      <span class="${CssClasses.SIDE_PANEL_CHECKMARK} ${
      isCurrLvl ? CssClasses.SIDE_PANEL_CHECKMARK_ACTIVE : ''
    }">${generateCheckMarkIcon(
      isCurrLvl?.isHintUsed ?? false,
      isCurrLvl ? CssClasses.CHECK_ICON_ACTIVE : '',
    )}</span>
            <span class="${CssClasses.SIDE_PANEL_LEVEL_NUM}">${(i + 1)
      .toString()
      .padStart(2, '0')}</span>
            <p class="${CssClasses.SIDE_PANEL_DESCR}">${
      data.levelTitles[i].title
    }</p>
        </button>
      </li>
    `;
...
const generateInitialMarkup = () => {
  ...
  levelsHeader.classList.add(CssClasses.SIDE_PANEL_HEADER);
  levelsHeader.textContent = LEVELS_HEADER;

  const resetProgressBtn = document.createElement('button');
  resetProgressBtn.classList.add(
    CssClasses.SIDE_PANEL_RESET_PROGRESS,
    CssClasses.BTN,
    CssClasses.BTN_ROUNDED,
  );
  resetProgressBtn.textContent = RESET_PROGRESS_BTN_TEXT;

  parentElement.classList.add(CssClasses.SIDE_PANEL_LEVELS);

  markup.append(levelsHeader, parentElement, resetProgressBtn);
  return markup;
};
...
const generateFooterMarkup = () => {
  ...
  footerWrapper.append(githubLink, rsschoolLogo, year);

  return footerWrapper;
};

export const addResetProgressHandler = (handler: MouseEventHandler) =>
  navWrapper.addEventListener('click', handler);

export const addChooseLevelsHandler = (handler: MouseEventHandler) =>
  parentElement.addEventListener('click', handler);

export const init = () => {
  navWrapper.classList.add(CssClasses.SIDE_PANEL);

  const footer = generateFooterMarkup();

  const initialMarkup = generateInitialMarkup();
  navWrapper.append(initialMarkup, footer);
